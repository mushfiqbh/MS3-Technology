<section class="py-16 w-full bg-gray-100 dark:bg-gray-800">
    ...
    <div class="container mx-auto px-6 sm:px-8 lg:px-12">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-1">
            @foreach ($activities as $activity)
                <div class="w-full overflow-hidden group rounded-lg">
                    <a href="{{ route('activities.details', $activity->id) }}">
                        ...
                        <div
                            class="w-full mb-2 bg-gray-50 dark:bg-gray-800 flex flex-col sm:flex-row overflow-hidden min-w-full relative">
                            <span
                            ...
                            <img src="{{ $activity->images->first() ? asset('storage/' . $activity->images->first()->image_path) : asset('images/default.png') }}"
                                alt="{{ $activity->title }}"
                                class="min-w-full max-h-52 rounded-b-lg object-cover object-center hover:scale-105 transition-transform duration-300">
                        </div>
                    </a>
                </div>
            @endforeach
